...
def main(args):
    ...
    print("do predict on", args.omics_label)
    ...
    if len(omics_list) > 1:
        ...
        if omics_list == "CopyNumberVariation":
            ...
            datasetnames = [datasetnames[i] for i in range(
                len(datasetnames)) if i in list(cancer_sample_index[0])]
            samplenames = [samplenames[i] for i in range(
                len(samplenames)) if i in list(cancer_sample_index[0])]
        ...
        train_input_M = getData(train_feature_M, gap=args.embedding_dim)
        test_input_M = getData(test_feature_M, gap=args.embedding_dim)
        train_labels = np.array([samplenames[i] for i in range(
            len(samplenames)) if i not in test_dataset_name_index])
        test_labels = np.array([samplenames[i] for i in range(
            len(samplenames)) if i in test_dataset_name_index])

        le = LabelEncoder()
        le.fit(np.array(samplenames))
        train_labels = le.transform(train_labels)
        test_labels = le.transform(test_labels)
    ...
    h5py_len_feature_result_file = os.path.join(
        output_dir, "len_feature_result.h5")
    ...
    with torch.no_grad():
        for batch in valid_loader:
            ...
            batch_samplenames = np.array(
                [(le.classes_[i]).encode() for i in labels.cpu().detach().numpy()])
            asset_dict = {'features': latent[:, 1:, :].cpu().detach().numpy(),
                          'samplenames': batch_samplenames}
            save_hdf5(h5py_len_feature_result_file, asset_dict, mode=mode)
            mode = 'a'
