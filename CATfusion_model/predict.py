...
def main(args):
    ...
    print("lifelines log rank test p value", results.p_value)        # 0.7676
    ...
    print("cumulative_dynamic_auc_y_data", cumulative_dynamic_auc_y_data[:5])
    va_times = np.arange(1, 8000, 100)
    ...
    svs_input_for_model_df.to_csv(os.path.join(
        output_dir, "svs_input_for_model_df.tsv"), sep="\t", header=True, index=False)
